...
public class LanguageData(long id, string name, LanguageTypes type) : Entity(id, name)
{
  ...
  public LanguageTypes Type { get; private set; } = type;
  ...
  internal string DoPreConversion(int percent, string text)
  {
    ...
    foreach (LanguageConversionData lcd in PreConversion.Where(lcd =>
               preConversion.Contains(lcd.OldValue) && Random.D100(1) >= percent))
    {
  ...
  internal string DoPostConversion(int percent, string text)
  {
    ...
    foreach (LanguageConversionData lcd in Conversion.Where(lcd =>
               postConversion.Contains(lcd.OldValue) && Random.D100(1) > percent))
    {
  ...
  private const string ENGLISH_ALPHABET = "abcdefghijklmnopqrtsuvwxyz";

  internal char GetAlphabetEquivalent(char englishChar)
  {
    char newChar = Alphabet[ENGLISH_ALPHABET.IndexOf(char.ToLower(englishChar))];
