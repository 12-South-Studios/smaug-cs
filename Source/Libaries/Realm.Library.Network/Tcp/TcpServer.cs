...
namespace Realm.Library.Network.Tcp
{
    ...
    public sealed class TcpServer : ITcpServer, INetworkServer
    {
        ...
        public TcpServer(NetworkSettings settings, ILogWrapper log, IUserRepository<string, TcpUser> repository, IEnumerable<IFormatter> formatters)
        {
            _settings = settings;
        ...
        private void HandleClientCommunication(object client)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    catch (ObjectDisposedException) { /* do nothing here, user disconnected */ }
                    ...
                    var eventArgs = new NetworkEventArgs { Message = encoder.GetString(message, 0, bytesRead) };
