...
public class CharacterInstanceExtensionTests
{
  ...
  {
    PlayerInstance pch = new(1, "TestChar");
  ...
  {
    PlayerInstance pch = new(1, "TestChar");
    MobileInstance mob = new(2, "TesterMob");
  ...
  {
    ILookupManager mockLookup = A.Fake<ILookupManager>();
  ...
  {
    MobileInstance mob = new(1, "TestChar");
    mob.Act.SetBit((int)ActFlags.IsNpc);
  ...
  {
    GenericRepository<ClassData> classRepo = new();
    classRepo.Add(1, new ClassData(1, "Test") { IsSpellcaster = isSpellcaster, Type = clsType });

    IRepositoryManager mockDb = A.Fake<IRepositoryManager>();
