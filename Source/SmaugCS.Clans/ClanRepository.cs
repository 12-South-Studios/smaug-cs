...
public class ClanRepository(ILogManager logManager, IDbContext dbContext) : IClanRepository
{
  ...
  public void Load()
  {
    try
    {
      if (dbContext.Count<Clan>() == 0) return;

      IEnumerable<Clan> clans = dbContext.GetAll<Clan>();
      foreach (Clan clan in clans)
      {
        ClanData newClan = new(clan.Id, clan.Name);
        ...
        ClanStats clanStats = clan.Stats;
        ...
        List<RosterData> members = [];
        members.AddRange(clan.Members.Select(clanMember => new RosterData()
        {
            Name = clanMember.Name,
            Joined = clanMember.Joined,
            Class = clanMember.ClassId,
            Level = clanMember.Level,
            Kills = clanMember.Kills,
            Deaths = clanMember.Deaths
        }));
        ...
        ClanMember member = clan.Members.FirstOrDefault(x => x.ClanRank == (int)ClanRanks.NumberOne);
      ...
      logManager.Boot("Loaded {0} Clans", Clans.Count());
    }
  ...
  public void Save()
  {
    try
    {
      foreach (ClanData clan in Clans.Where(x => !x.Saved).ToList())
      {
        Clan clanToSave = dbContext.Get<Clan>(clan.Id) ?? new Clan
        {
            Name = clan.Name,
            ClanType = (int)clan.ClanType
        };
        ...
        {
          ClanMember clanMember = clanToSave.Members.FirstOrDefault(x => x.Name == member.Name);
          if (clanMember == null)
          {
        ...
        dbContext.AddOrUpdate(clanToSave);
      }
    }
    catch (DbException ex)
    {
  ...
  private ClanRanks GetClanRank(string name, ClanData clan)
  {
    ...
    return clan.NumberTwo == name ? ClanRanks.NumberTwo : ClanRanks.Member;
  }
