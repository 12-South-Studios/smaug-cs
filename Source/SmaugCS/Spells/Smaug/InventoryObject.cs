...
public static class InventoryObject
{
    ...
    switch (Macros.SPELL_ACTION(skill))
    {
      case (int)SpellActTypes.Create:
        ...
        if (Macros.SPELL_CLASS(skill) == (int)SpellClassTypes.Life
            &&
            obj.ItemType is ItemTypes.Food or ItemTypes.Cook or ItemTypes.DrinkContainer)
  ...
  private static ReturnTypes CloneObjectSpellAction(SkillData skill, int level, CharacterInstance ch,
    ObjectInstance obj)
  {
    switch (Macros.SPELL_POWER(skill))
    {
      ...
      case (int)SpellPowerTypes.Greater:
        if (CheckFunctions.CheckIfTrueCasting(ch.Level - obj.Level < 5 ||
                                              obj.Cost > ch.Level * 10 * ch.GetCurrentIntelligence() *
                                              ch.GetCurrentWisdom(),
              skill, ch, CastingFunctionType.Failed, null, obj)) return ReturnTypes.None;
        break;
      case (int)SpellPowerTypes.Major:
        if (CheckFunctions.CheckIfTrueCasting(ch.Level - obj.Level < 0 ||
                                              obj.Cost > ch.Level * 50 * ch.GetCurrentIntelligence() *
                                              ch.GetCurrentWisdom(),
              skill, ch, CastingFunctionType.Failed, null, obj)) return ReturnTypes.None;
  ...
  private static ReturnTypes CreateWaterSpellAction(SkillData skill, int level, CharacterInstance ch,
    ObjectInstance obj, WeatherCell cell)
  {
    if (CheckFunctions.CheckIfTrue(ch, obj.ItemType != ItemTypes.DrinkContainer, "It is unable to hold water."))
      return ReturnTypes.SpellFailed;
    if (CheckFunctions.CheckIfTrue(ch, obj.Value.ToList()[2] != 0 && obj.Value.ToList()[1] != 0,
    ...
    int water = minVal.GetLowestOfTwoNumbers(obj.Value.ToList()[0] - obj.Value.ToList()[1]);
  ...
  {
    if (Macros.SPELL_DAMAGE(skill) != (int)SpellDamageTypes.Poison) return ReturnTypes.None;
