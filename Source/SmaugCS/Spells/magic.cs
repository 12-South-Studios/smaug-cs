...
public static class magic
{
  ...
  {
    SkillData skill = Program.RepositoryManager.GetEntity<SkillData>(name);
  ...
  public static int slot_lookup(int slot)
  {
    if (slot <= 0)
      return -1;
    foreach (SkillData skill in Program.RepositoryManager.SKILLS.Values.Where(skill => skill.Slot == slot))
      return (int)skill.Id;
    return -1;
  }
  ...
  /// </summary>
  public static int dispel_casting(AffectData paf, CharacterInstance ch, CharacterInstance victim, int affect,
    bool dispel)
  {
    bool isMage = false;
    ...
    foreach (SpellComponent component in skill.Components)
    {
      ...
      switch (component.RequiredType)
      {
        case ComponentRequiredTypes.ItemType:
          foreach (ObjectInstance vobj in ch.Carrying.Where(vobj => vobj.ItemType ==
        ...
        case ComponentRequiredTypes.ItemVnum:
          foreach (ObjectInstance vobj in ch.Carrying.Where(vobj => vobj.Id == component.RequiredData.ToInt32()))
          {
            if (CheckFunctions.CheckIfTrue(ch, fail,
                  "Something disrupts the casting of this spell...")) return false;
            found = true;
        ...
        case ComponentRequiredTypes.ItemKeyword:
          foreach (ObjectInstance vobj in ch.Carrying.Where(vobj => vobj.Name.IsAnyEqual(component.RequiredData)))
          {
            if (CheckFunctions.CheckIfTrue(ch, fail,
                  "Something disrupts the casting of this spell...")) return false;
            found = true;
      ...
      if (val >= 0)
      {
        ...
        if (--obj.Value.ToList()[val] == 0)
        {
          comm.act(ATTypes.AT_MAGIC, "$p glows briefly, then disappears in a puff of smoke!", ch, obj, null,
            ToTypes.Character);
          comm.act(ATTypes.AT_MAGIC, "$p glows briefly, then disappears in a puff of smoke!", ch, obj, null,
            ToTypes.Room);
          obj.Extract();
  ...
  public static object locate_targets(CharacterInstance ch, string arg, int sn, CharacterInstance victim,
    ObjectInstance obj)
  {
    SkillData skill = Program.RepositoryManager.GetEntity<SkillData>(sn);
    if (skill == null)
  ...
  private static object TargetCharacterWithOffensiveSpell(string arg, CharacterInstance ch, bool silence,
    SkillData skill)
  {
    ...
    if (!ch.IsNpc() && ch.CurrentFighting != null && ((PlayerInstance)ch).PlayerData.Nuisance != null
        ...
        SmaugRandom.D100() < (((PlayerInstance)ch).PlayerData.Nuisance.Flags - 5) * 8 +
        6 * ((PlayerInstance)ch).PlayerData.Nuisance.Power)
      victim = ch.GetMyTarget();
  ...
  private static object TargetSelf(string arg, CharacterInstance ch, bool silence)
  {
    return CheckFunctions.CheckIfTrue(ch, !arg.IsNullOrEmpty() && !arg.EqualsIgnoreCase(ch.Name),
      !silence ? "You cannot cast this spell on another." : "")
      ? null
      : ch;
  }
    ...
    CharacterInstance victim = null, ObjectInstance obj = null)
  {
    ...
    if (skill.Target != TargetTypes.OffensiveCharacter
        || victim == ch
        || victim.CharDied()) return retcode;
    foreach (
