...
public static class Cast
{
  ...
  private static bool DefaultCast(CharacterInstance ch, string argument)
  {
    ...
    SpellFunction spell = Program.LookupManager.SpellLookup.Get(arg1);
    SkillData spellSkill = Program.RepositoryManager.GetEntity<SkillData>(arg1);
    ...
    if (ch.Trust < GetGodLevel())
    {
      ...
      PlayerInstance pc = ch as PlayerInstance;
      int skillMastery = spellSkill.GetMasteryLevel(pc);
  ...
  private static bool CastAbortTimer(PlayerInstance ch, string argument)
  {
    ...
    if (Macros.IS_VALID_SN(sn))
    {
      ...
      int mana = ch.IsNpc() ? 0 : Macros.UMAX(skill.MinimumMana, 100 / (2 + ch.Level - skillLevel));
      int blood = Macros.UMAX(1, (mana + 4) / 8);
