...
public static class Flee
{
  ...
  private static bool MakeFleeAttempt(CharacterInstance ch, RoomTemplate wasIn)
  {
    ...
    if (exit?.GetDestination() == null || exit.Flags.IsSet(ExitFlags.NoFlee) || exit.Flags.IsSet(ExitFlags.Closed) ||
        !ch.IsAffected(AffectedByTypes.PassDoor) || (ch.IsNpc() && exit.GetDestination().Flags.IsSet(RoomFlags.NoMob)))
      return false;

    SkillData sneak = Program.RepositoryManager.GetEntity<SkillData>("sneak");
