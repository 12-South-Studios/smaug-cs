...
public static class Move
{
  public static ReturnTypes move_char(CharacterInstance ch, ExitData pexit, int fall)
  {
    ...
    if (!ch.IsNpc())
    {
      ...
      if (pch.PlayerData.Nuisance != null && pch.PlayerData.Nuisance.Flags > 8
                                          ...
                                          && ch.CurrentPosition != PositionTypes.Drag
                                          && SmaugRandom.D100() > pch.PlayerData.Nuisance.Flags *
                                          pch.PlayerData.Nuisance.Power)
        nuisance = true;
    }
                ...
                && ch.CurrentRoom.SectorType != SectorTypes.OceanFloor)
      {
        switch (SmaugRandom.Bits(4))
        {
          default:
            ...
            break;
          case 3:
            comm.act(ATTypes.AT_ACTION, "In your drunken stupor you trip over your own feet and tumble to the ground.",
              ch, null, null, ToTypes.Character);
            comm.act(ATTypes.AT_ACTION, "$n stumbles drunkenly, trips and tumbles to the ground.", ch, null, null,
              ToTypes.Room);
            ch.CurrentPosition = PositionTypes.Resting;
            break;
          case 4:
            comm.act(ATTypes.AT_SOCIAL, "You utter a string of slurred obscenities.", ch, null, null,
              ToTypes.Character);
            comm.act(ATTypes.AT_ACTION, "Something blurry and immovable has intercepted you as you stagger along.", ch,
              null, null, ToTypes.Character);
            comm.act(ATTypes.AT_HURT, "Oh geez... THAT really hurt.  Everything slowly goes dark and numb...", ch, null,
              null, ToTypes.Character);
            comm.act(ATTypes.AT_ACTION, "$n drunkenly staggers into something.", ch, null, null, ToTypes.Room);
            comm.act(ATTypes.AT_SOCIAL, "$n utters a string of slurred obscenities: @*&^%@*&!", ch, null, null,
              ToTypes.Room);
        ...
        comm.act(ATTypes.AT_ACTION, "You stare around trying to remember where you where going.", ch, null, null,
          ToTypes.Character);
      else if (drunk)
        comm.act(ATTypes.AT_ACTION, "You stare around trying to make sense of things through your drunken stupor.", ch,
          null, null, ToTypes.Character);
      else
    ...
    if (exit.Flags.IsSet(ExitFlags.Closed) &&
        (!ch.IsAffected(AffectedByTypes.PassDoor) || exit.Flags.IsSet(ExitFlags.NoPassDoor)))
    {
    ...
    if (CheckFunctions.CheckIfTrue(ch, fall == 0 && ch.IsAffected(AffectedByTypes.Charm)
                                                 && ch.Master != null && inRoom == ch.Master.CurrentRoom,
    ...
    if (fall == 0 && !ch.IsNpc())
    {
      ...
      if (toRoom.Area.Flags.IsSet(AreaFlags.NoPlayerVsPlayer) && !ch.CurrentRoom.Area.Flags.IsSet(AreaFlags
                                                                .NoPlayerVsPlayer)
  ...
  private static readonly Dictionary<PositionTypes, string> PositionMoveMessage = new()
  {
