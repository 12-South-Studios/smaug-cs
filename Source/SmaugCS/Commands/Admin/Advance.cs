...
public static class Advance
{
  public static void do_advance(CharacterInstance ch, string argument)
  {
    ...
    int level = Convert.ToInt32(secondArg);
  ...
  private static void RaiseVictimLevel(CharacterInstance ch, CharacterInstance victim, int level)
  {
    ...
    try
    {
      ImmortalTypes immortalType = EnumerationExtensions.GetEnum<ImmortalTypes>(level);

      ImmortalHelpCategoryAttribute attrib =
        Common.EnumerationExtensions.GetAttribute<ImmortalHelpCategoryAttribute>(immortalType);
      if (attrib != null)
      ...
      while (victim.Carrying.Count != 0)
      {
        ObjectInstance objInstance = victim.Carrying.FirstOrDefault();
        objInstance?.Extract();
      }
    }
  ...
  private static void LowerVictimLevel(CharacterInstance ch, CharacterInstance victim, int level)
  {
    ...
    if (victim.Level >= LevelConstants.AvatarLevel && victim.IsImmortal())
    {
      ...
      if (victimPc.PlayerData.Bestowments.Count != 0)
    ...
    if (victim is PlayerInstance)
    {
      PlayerInstance player = victim as PlayerInstance;
