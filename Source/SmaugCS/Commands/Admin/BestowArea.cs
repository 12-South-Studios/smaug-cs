...
public static class BestowArea
{
  public static void do_bestowarea(CharacterInstance ch, string argument)
  {
    ...
    {
      IEnumerable<string> areaNames = ((PlayerInstance)victim).PlayerData.Bestowments.Where(x => x.EndsWith(".are"));
    ...
    {
      ((PlayerInstance)victim).PlayerData.Bestowments =
        ((PlayerInstance)victim).PlayerData.Bestowments.Where(x => !x.EndsWith(".are")).ToList();
      ch.SendTo("Done.");
      return;
    }
