...
public static class MPApply
{
  public static void do_mpapply(CharacterInstance ch, string argument)
  {
    if (CheckFunctions.CheckIfTrue(ch,
          !ch.IsNpc() || ((PlayerInstance)ch).Descriptor != null || ch.IsAffected(AffectedByTypes.Charm),
          "Huh?")) return;
    ...
    if (CheckFunctions.CheckIfNullObject(ch, !victim.IsNpc() && ((PlayerInstance)victim).Descriptor != null,
          "Not on link-dead players")) return;
    ...
    string buf =
      $"{victim.Name}@{(victim.IsNpc() ? string.Empty : ((PlayerInstance)victim).Descriptor.host)} " +
      $"new {victim.CurrentRace.GetName()} {victim.CurrentClass.GetName()} {(victim.IsPKill() ? "(Deadly)" : "(Peaceful)")} applying...";
