...
public static class Worth
{
  ...
  private static readonly List<LookupLevel> DeityFavorList =
  [
  ...
  private static readonly List<LookupLevel> AlignmentList =
  [
  ...
  public static void do_worth(CharacterInstance ch, string argument)
  {
    ...
    buffer = string.Format(buffer,
      ((PlayerInstance)ch).PlayerData.quest_curr.ToString(CultureInfo.InvariantCulture).PadLeft(4),
  ...
  private static string GetDeityFavor(CharacterInstance ch)
  {
    ...
    DeityFavorList.Sort();
    LookupLevel deityLookup =
      DeityFavorList.FirstOrDefault(level => ((PlayerInstance)ch).PlayerData.Favor > level.Level);
