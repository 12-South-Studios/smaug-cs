...
public static class Group
{
  ...
  private static void display_group(CharacterInstance ch)
  {
    ...
    foreach (CharacterInstance gch in Program.RepositoryManager.CHARACTERS.Values.Where(x => x.IsSameGroup(ch)))
    {
      ch.SetColor(ATTypes.AT_DGREEN);
  ...
  private static void disband_group(CharacterInstance ch)
  {
    ...
    int count = 0;
    foreach (CharacterInstance gch in Program.RepositoryManager.CHARACTERS.CastAs<Repository<long, CharacterInstance>>()
               .Values.Where(x => x.IsSameGroup(ch) && x != ch))
    {
      gch.Leader = null;
      gch.Master = null;
      count++;
