...
public static class Tell
{
  public static void do_tell(CharacterInstance ch, string argument)
  {
    ...
    {
      comm.act(ATTypes.AT_PLAIN, "$E is too tired to discuss such matters with you.", ch, null, victim,
        ToTypes.Character);
      return;
    }
    ...
    {
      comm.act(ATTypes.AT_PLAIN, "A magic force prevents your message from being heard.", ch, null, victim,
        ToTypes.Character);
      return;
    }
    ...
    {
      comm.act(ATTypes.AT_PLAIN, "$E is currently in a writing buffer. Please try again later.", ch, null, victim,
        ToTypes.Character);
      return;
    }
    ...
    MudProgHandler.ExecuteMobileProg(Program.Container.Resolve<IMudProgHandler>(), MudProgTypes.Tell, argumentString,
      ch);
  }
}