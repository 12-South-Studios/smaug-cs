...
public static class skills
{
  ...
  public static CharacterInstance scan_for_victim(CharacterInstance ch, ExitData pexit, string name)
  {
    ...
    RoomTemplate was_in_room = ch.CurrentRoom;
    int max_distance = 8;
    ...
    for (int dist = 1; dist <= max_distance; dist++)
    {
      ...
      CharacterInstance victim = ch.GetCharacterInRoom(name);
      if (victim != null)
      {
        ch.CurrentRoom.RemoveFrom(ch);
      ...
      DirectionTypes dir = pexit.Direction;
      ExitData exit = ch.CurrentRoom.GetExit(dir);
      if (exit == null)
        break;
    }
  ...
  {
    foreach (ObjectInstance obj in ch.Carrying.Where(ch.CanSee))
    {
      if (obj.ItemType == ItemTypes.Quiver
          && !obj.Values.Flags.IsSet(ContainerFlags.Closed))
      {
        foreach (ObjectInstance containedObj in obj.Contents
    ...
    ATTypes color)
  {
    if (ch.CurrentRoom.Flags.IsSet(RoomFlags.Safe))
    {
      ...
      else
      {
        ...
        comm.act(color, "A godly presence smites $t!", ch, stxt.AddArticle(ArticleAppendOptions.TheToFront), null,
          ToTypes.Room);
      }
    ...
    SkillData skill = Program.RepositoryManager.GetEntity<SkillData>("MissileWeapons");
                                                       ...
                                                       && ch.CanUseSkill(SmaugRandom.D100(), skill)))
    {
      return projectile != null
        ? fight.projectile_hit(ch, victim, weapon, projectile, dist)
        : Attack.spell_attack(dt, ch.Level, ch, victim);
    }

    skill.LearnFromFailure(ch);
    ReturnTypes returnCode = ch.CauseDamageTo(victim, 0, dt);
    ...
    ObjectInstance projectile, int sn, int range)
  {
    ...
    ExitData exit = ch.CurrentRoom.GetExit(firstArg);
    if (exit == null)
    {
    ...
    if (projectile != null)
    {
      ...
      string action = weapon != null ? "fire" : "throw";
      if (exit != null)
      {
    ...
    {
      string skillText = GetSkillText(skill);

      if (skill.Type == SkillTypes.Spell)
      {
        ATTypes color = ATTypes.AT_MAGIC;
        if (exit != null)
        {
    ...
    victim = vch;
    string dtxt = act_move.rev_exit(exit.Direction);
    ...
    while (dist <= range)
    {
      ...
      {
        ATTypes color = projectile != null ? ATTypes.AT_GREY : ATTypes.AT_MAGIC;
  ...
  {
    string skillText = !string.IsNullOrEmpty(skill.NounDamage) ? skill.NounDamage : skill.Name;
