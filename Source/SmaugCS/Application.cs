...
public class Application
{
  ...
  public Application(IServiceProvider serviceProvider)
  {
    ...
    ((ITcpServer)_server).OnTcpUserStatusChanged += NetworkManager_OnOnTcpUserStatusChanged;
  }
  ...
  private static void InitializeManagersAndGameSettings(Config.Configuration.Constants constants,
    Config.Configuration.Settings settings)
  {
    ...
    IInitializer luaInitializer = Program.Container.ResolveNamed<IInitializer>("LuaInitializer");
  ...
  private static void InitializeStaticGameData(Config.Configuration.Constants constants,
    Config.Configuration.Settings settings)
  {
    Program.LogManager.Boot("Initializing Game Data");
    LoadSystemDataFromLuaScripts(constants, settings);
    IInitializer loaderInitializer = Program.Container.ResolveNamed<IInitializer>("LoaderInitializer");
  ...
  private static void LoadSystemDataFromLuaScripts(Config.Configuration.Constants constants,
    Config.Configuration.Settings settings)
  {
    ...
    foreach (string language in settings.Languages.Split(","))
    {
  ...
  {
    INetworkUser user = (INetworkUser)sender;
    TcpNetworkEventArgs eventArgs = (TcpNetworkEventArgs)networkEventArgs;
  ...
  {
    DescriptorData descrip = new(9999, 9999, 9999) { User = user };
    db.DESCRIPTORS.Add(descrip);
  }

  private static void DisconnectUser(INetworkUser user)
  {
    PlayerInstance character = Program.RepositoryManager.CHARACTERS.Values.OfType<PlayerInstance>()
      .FirstOrDefault(x => x.Descriptor.User == user);
    if (character == null)
    {
