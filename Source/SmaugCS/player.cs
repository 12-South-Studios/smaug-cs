...
{
  private static readonly Dictionary<int, string> affect_locations = new()
  {
  ...
  {
    return affect_locations.GetValueOrDefault(location, "(?)");
  }
  ...
  public static string get_class(CharacterInstance ch)
  {
    ...
    if (ch.IsNpc()) return "Unknown";
    ClassData cls = Program.RepositoryManager.GetClass(ch.CurrentClass);
    return cls.Name;
  ...
  {
    ch.PlayerData.Title = char.IsLetterOrDigit(title[0]) ? title[1..] : title;
  }
}