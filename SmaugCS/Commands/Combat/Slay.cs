...
namespace SmaugCS.Commands.Combat
{
    ...
    {
        private static readonly Dictionary<string, Action<CharacterInstance, CharacterInstance>> SlayTable = new Dictionary
            <string, Action<CharacterInstance, CharacterInstance>>
        {
            {"immolate", Immolate},
            {"shatter", Shatter},
            {"demon", Demon},
        ...
        private static void Immolate(CharacterInstance ch, CharacterInstance victim)
        {
            ...
            comm.act(ATTypes.AT_FIRE, "$n points at $N, who bursts into a flaming inferno.", ch, null, victim, ToTypes.NotVictim);
        }
        private static void Shatter(CharacterInstance ch, CharacterInstance victim)
        {
            comm.act(ATTypes.AT_LBLUE, "You freeze $N with a glance and shatter the frozen corpse into tiny shards.", ch, null, victim, ToTypes.Character);
            comm.act(ATTypes.AT_LBLUE, "$n freezes you with a glance and shatters your frozen body into tiny shards.", ch, null, victim, ToTypes.Victim);
            comm.act(ATTypes.AT_LBLUE, "$n freezes $N with a glance and shatters $S frozen body into tiny shards.", ch, null, victim, ToTypes.NotVictim);
        }
        private static void Demon(CharacterInstance ch, CharacterInstance victim)
        {
            comm.act(ATTypes.AT_IMMORT, "You gesture, and a slavering demon appears.  With a horrible grin, the", ch, null, victim, ToTypes.Character);
            comm.act(ATTypes.AT_IMMORT, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, null, victim,
                 ToTypes.Character);
            comm.act(ATTypes.AT_IMMORT, "$n gestures, and a slavering demon appears.  The foul creature turns on", ch, null, victim, ToTypes.Victim);
            comm.act(ATTypes.AT_IMMORT, "you with a horrible grin.   You scream in panic before being eaten alive.", ch, null, victim,
                 ToTypes.Victim);
            comm.act(ATTypes.AT_IMMORT, "$n gestures, and a slavering demon appears.  With a horrible grin, the", ch, null, victim,
                 ToTypes.NotVictim);
            comm.act(ATTypes.AT_IMMORT, "foul creature turns on $N, who screams in panic before being eaten alive.", ch, null, victim,
                 ToTypes.NotVictim);
        }
        ...
        {
            comm.act(ATTypes.AT_BLOOD, "Leaping upon $N with bared fangs, you tear open $S throat and toss the corpse to the ground...", ch, null, victim, ToTypes.Character);
            comm.act(ATTypes.AT_BLOOD, "In a heartbeat, $n rips $s fangs through your throat!  Your blood sprays and pours to the ground as your life ends...", ch, null, victim, ToTypes.Victim);
            comm.act(ATTypes.AT_BLOOD, "Leaping suddenly, $n sinks $s fangs into $N's throat.  As blood sprays and gushes to the ground, $n tosses $N's dying body away.", ch, null, victim, ToTypes.NotVictim);
        }
        private static void Slit(CharacterInstance ch, CharacterInstance victim)
        {
            ...
            comm.act(ATTypes.AT_BLOOD, "$n reaches out with a clawed finger and calmly slits your throat.", ch, null, victim, ToTypes.Victim);
